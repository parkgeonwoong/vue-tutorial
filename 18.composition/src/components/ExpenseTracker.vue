...
export default {
  setup() {
    ...
    const remainingFunds = computed(() => {
      return data.value.availableFunds - data.value.currentExpenses;
    });

    const addExpense = () => {
      data.value.currentExpenses += data.value.enteredExpense;
      data.value.enteredExpense = 0;
    };
    ...
    return { data, remainingFunds, addExpense };
  }
